...
import {
  getChamSocPhucHoiToc,
  getServicesByCategoryAndType,
...
export default function ChamSocPhucHoiTocPage() {
  const [selectedType, setSelectedType] = useState("all")
  const allServices = getChamSocPhucHoiToc()
  const availableTypes = getServiceTypesForCategory("cham-soc-phuc-hoi-toc")

  const filteredServices = useMemo(() => {
    return getServicesByCategoryAndType("cham-soc-phuc-hoi-toc", selectedType)
  }, [selectedType])
  ...
  return (
    <div className="min-h-screen">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center">
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-6">
              Chăm Sóc & Phục Hồi Tóc
            </h1>
            <p className="text-xl text-gray-600 max-w-3xl mx-auto">
              Dịch vụ chăm sóc & phục hồi tóc chuyên nghiệp, giúp mái tóc chắc khỏe,
              bóng mượt và phục hồi hư tổn hiệu quả
            </p>
          </div>
        </div>
      </section>
      ...
      <ServiceTypeSelect
        ...
        placeholder="Tất cả dịch vụ chăm sóc & phục hồi tóc"
      />
            ...
            <>
              <div className="text-center mb-8">
                <p className="text-gray-600">
                  Hiển thị{" "}
                  <span className="font-semibold text-orange-600">{filteredServices.length}</span>{" "}
                  dịch vụ
                  {selectedType !== "all" && (
                    <span>
                      {" "}
